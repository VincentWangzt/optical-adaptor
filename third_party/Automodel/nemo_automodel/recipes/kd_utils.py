...
def _model_replicas(setup: DistributedSetup, ranks: tuple[int, ...]) -> list[_Replica]:
    if isinstance(setup.strategy_config, DDPConfig):
        return [_Replica((rank,), rank, rank) for rank in ranks]
...
class KDMeshBridge:
    ...
    def __init__(self, setups: KDDistributedSetups, *, device: torch.device) -> None:
        ...
        self.student_replicas = _model_replicas(setups.student, setups.student_ranks)
        self.teacher_replicas = _model_replicas(setups.teacher, setups.teacher_ranks)
