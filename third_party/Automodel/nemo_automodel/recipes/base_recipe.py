...
class BaseRecipe:
    ...
    def _get_dp_group(self, include_cp: bool = False):
        ...
        if not device_mesh:
            return getattr(self, "_training_process_group", None)
    ...
    def _get_dp_group_size(self, include_cp: bool = False):
        ...
        if not device_mesh:
            if dist.is_initialized():
                return dist.get_world_size(getattr(self, "_training_process_group", None))
            return 1
    ...
    def _get_dp_rank(self, include_cp: bool = False):
        ...
        if not device_mesh:
            ...
            if dist.is_initialized():
                return dist.get_rank(getattr(self, "_training_process_group", None))
            return 0
